...
- **All 12 HTML pages** — Added a `<main id="main">` landmark wrapping the primary content (between `</header>` and `<footer>`). Fixes the "document lacks main landmark" accessibility/AEO finding. Verified no CSS depends on parent-child structure, so layout is unchanged.
- **index.html** — Added `AggregateRating` (5.0, 5 reviews) + 5 `Review` objects to the existing LocalBusiness JSON-LD, sourced verbatim from the reviews already displayed on the page. Replaced generic link text: "View Our Work" → "View Our Project Gallery"; "View Service Options" → "View Screen, Glass & Hardware Service"; "View Gallery" → "View the Full Project Gallery".
- **installation.html** — Added "in Southern California" geo modifier to `<title>`, `og:title`, `twitter:title`, and the H1.
- **service-maintenance.html** — Added "in Southern California" geo modifier to `<title>`, `og:title`, `twitter:title`, and the H1.
- **large-opening-screens.html, lift-and-slide-door-installation.html, specialty-window-door-repair.html** — Added `FAQPage` JSON-LD generated directly from each page's existing visible FAQ Q&A (7, 4, and 5 questions respectively). Sales and Installation already had FAQ schema.
- **llms.txt** — New file at site root: business summary, services, key pages, and contact for AI/answer-engine crawlers.
- **sitemap.xml** — Appended the 3 specialty landing-page URLs (previously missing).

## Indexing

- **`noindex` → `index, follow` on all 7 core pages** (index, sales, installation, service-maintenance, projects, about, contact) — the audit's #1 action, approved by owner for production launch. Removed the stale `STAGING ONLY` code comment on those pages. **404.html and thank-you.html intentionally remain `noindex`** (error and post-submit confirmation pages should never be indexed). Deploy target must be the production domain (visiblewindowsanddoors.com), not a preview URL.

## Deliberately NOT changed

- **Street address / NAP** — Owner elected to keep service-area-only; no address added.
